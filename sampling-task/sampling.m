...
% lbl_pre = [0*ones(N,1);ones(N,1);2*ones(N,1);3*ones(N,1);4*ones(N,1);5*ones(N,1);6*ones(N,1);7*ones(N,1);8*ones(N,1);9*ones(N,1)];
% % p = randperm(10000);
% % 
% % img = img_pre(:,p);
% % lbl = lbl_pre(p);
...
for i = 1:N
    for j = 1:N
        dist(i,j) = sqrt((img_final(:,i)-img_final(:,j))'*(img_final(:,i)-img_final(:,j)));
    end
end
